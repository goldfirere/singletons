...
promoteDecs decls = do
  ...
  mapM_ (\n -> report False $ "No type binding for " ++ (show (nameBase n)) ++
...
promoteTopLevelPat (TildeP pat) = do
  lift $ report False "Lazy pattern converted into regular pattern in promotion"
  promoteTopLevelPat pat
promoteTopLevelPat (BangP pat) = do
  lift $ report False "Strict pattern converted into regular pattern in promotion"
  promoteTopLevelPat pat
...
promoteTopLevelPat (SigP pat _) = do
  lift $ report False $ "Promotion of explicit type annotation in pattern " ++
                        "not yet supported."
  promoteTopLevelPat pat
