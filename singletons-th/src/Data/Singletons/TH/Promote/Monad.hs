...
When promoting the `j` function to a type family `J`, there are two possible
ways of promoting `x`:

* If `x` is a lambda-bound variable, then `x` must be promoted to the same
  name, since `x`'s binding site will also use that name. For instance, we
  would promote this:
