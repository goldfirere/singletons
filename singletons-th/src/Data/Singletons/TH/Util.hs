...
freshenTyVarsM = everywhereM freshenTyVarsStep
  where
    ...
    fix_local_var :: LocalVar -> State FreshenState LocalVar
    fix_local_var (LocalVar { lvName = n, lvKind = mbKind }) = do
      n' <- freshenName n
...
non-unique Names. In each freshening-scope, all variables encountered are given
an incrementing unique name. For example, `x_456` will
return a non-unique Name with the OccName `x0`. We use this renamer when
generating `LetG` so
that it will instead be:

    type family LetG x0 where
      LetG x0 = x0

Here, `x0` is a non-unique Name, and `x_456` is a Unique name. Thankfully,
