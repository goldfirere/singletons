...
data SuccSym0 :: TyFun Nat Nat -> Type
...
data ConsSym0 :: TyFun a (TyFun (List a) (List a) -> Type) -> Type
type instance Apply ConsSym0 a = ConsSym1 a

data ConsSym1 :: a -> TyFun (List a) (List a) -> Type
...
type family Map (f :: TyFun k1 k2 -> Type) (l :: List k1) :: List k2 where
    Map f Nil = Nil
    Map f (Cons h t) = Cons (Apply f h) (Map f t)

-- defunctionalization symbols
data MapSym1 (k1 :: TyFun a b -> Type)
             (k2 :: TyFun (List a) (List b))
data MapSym0 (k3 :: TyFun (TyFun a b -> Type)
                          (TyFun (List a) (List b) -> Type))
type instance Apply (MapSym1 f) xs = Map f xs
type instance Apply  MapSym0 f     = MapSym1 f

sMap :: forall (a :: List k1) (f :: TyFun k1 k2 -> Type).
...
sMap2 :: forall (a :: List k1) (f :: TyFun k1 k2 -> Type). Proxy f ->
...
type family ZipWith (k1 :: TyFun a (TyFun b c -> Type) -> Type)
                    (k2 :: List a)
...
data ZipWithSym2 (k1 :: TyFun a (TyFun b c -> Type) -> Type)
                 (k2 :: List a)
                 (k3 :: TyFun (List b) (List c))
data ZipWithSym1 (k1 :: TyFun a (TyFun b c -> Type) -> Type)
                 (k2 :: TyFun (List a) (TyFun (List b) (List c) -> Type))
data ZipWithSym0 (k3 :: TyFun (TyFun a (TyFun b c -> Type) -> Type)
                              (TyFun (List a)
                                     (TyFun (List b) (List c) -> Type) -> Type))
...
sZipWith :: forall (k1 :: TyFun a (TyFun b c -> Type) -> Type) (k2 :: List a) (k3 :: List b).
...
type family Either_ (l :: TyFun a c -> Type) (r :: TyFun b c -> Type) (e :: Either a b) :: c where
    Either_ l r (Left x) = Apply l x
    Either_ l r (Right x) = Apply r x

-- defunctionalization symbols
data Either_Sym2 (k1 :: TyFun a c -> Type)
                 (k2 :: TyFun b c -> Type)
                 (k3 :: TyFun (Either a b) c)
data Either_Sym1 (k1 :: TyFun a c -> Type)
                 (k2 :: TyFun (TyFun b c -> Type)
                              (TyFun (Either a b) c -> Type))
data Either_Sym0 (k1 :: TyFun (TyFun a c -> Type)
                              (TyFun (TyFun b c -> Type)
                                     (TyFun (Either a b) c -> Type) -> Type))
...
sEither :: forall (l :: TyFun a c -> Type)
                  (r :: TyFun b c -> Type)
                  (e :: Either a b).
...
-- Not generated by the library.
sEither2 :: forall (l :: TyFun a c -> Type)
                   (r :: TyFun b c -> Type)
                   (e :: Either a b).
           Proxy l -> Proxy r ->
...
data LiftMaybeSym1 (k1 :: TyFun a b -> Type)
                   (k2 :: TyFun (Maybe a) (Maybe b))
data LiftMaybeSym0 (k1 :: TyFun (TyFun a b -> Type)
                                (TyFun (Maybe a) (Maybe b) -> Type))
type instance Apply (LiftMaybeSym1 k1) k2 = LiftMaybe k1 k2
type instance Apply  LiftMaybeSym0     k1 = LiftMaybeSym1 k1

sLiftMaybe :: forall (f :: TyFun a b -> Type) (x :: Maybe a).
...
data (+$$) (k1 :: Nat)
            (k2 :: TyFun Nat Nat)
data (+$)  (k1 :: TyFun Nat (TyFun Nat Nat -> Type))
...
data (||$$) (k1 :: Bool)
             (k2 :: TyFun Bool Bool)
data (||$)  (k1 :: TyFun Bool (TyFun Bool Bool -> Type))
...
data ContainsSym0 (k1 :: TyFun a (TyFun (List a) Bool -> Type))
...
type family Cont :: TyFun a (TyFun (List a) Bool -> Type) -> Type where
  Cont = Lambda10Sym0
...
type family FindIndices (f :: TyFun a Bool -> Type) (ls :: List a) :: List Nat where
...
sFindIndices :: forall (t1 :: TyFun a Bool -> Type) (t2 :: (List a)).
                Sing t1
             -> Sing t2
...
{-
sFI :: forall (t1 :: TyFun a Bool -> Type) (t2 :: List a). Sing t1
    -> Sing t2
    -> Sing (FISym0 @@ t1 @@ t2)
sFI = unSingFun2 (singFun2 @FI (\p ls ->
    let lambda :: forall {-(t1 :: TyFun a Bool -> Type)-} t1 t2. Sing t1 -> Sing t2 -> Sing (Lambda22Sym0 @@ t1 @@ t2)
        lambda sP sLs =
