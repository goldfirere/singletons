 ...
 * GHC doesn't use the in-tree singletons library. This means you need to
   install the library in your system before running the tests. You can use:

      ```bash
      make install && make tests
      ````

   This is a bit annoying since the library needs to be compiled twice - first
   to install it and then to for the second time to run tests. This behaviour
   can be slightly improved by using cabal sandbox feature added in the 1.18
   release. This will most likely not prevent double compilation but at least we
   won't have to install into global package database. Since cabal 1.18 is not
   yet in wide use we leave things as they are for the moment.
 ...
 * Testsuite allows to run compiled programs and compare their output with a
   golden file. See "Database client" for example of a test when a program is
   first compiled with GHC and then run. Template files are expected to have the
   extension `.run.template`. We assume that programs produce always the same
   output, no matter which GHC version was used to compile them.
...
are thus not part of the testsuite.
