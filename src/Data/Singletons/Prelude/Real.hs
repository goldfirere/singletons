{-# LANGUAGE TemplateHaskell, ScopedTypeVariables, DataKinds, PolyKinds,
             GADTs, TypeFamilies, TypeOperators, FlexibleContexts,
             UndecidableInstances, DefaultSignatures, InstanceSigs #-}

-----------------------------------------------------------------------------
-- |
-- Module      :  Data.Singletons.Prelude.Real
...
$(genSingletons [''Ratio])
$(singEqInstance ''Ratio)
$(singDecideInstance ''Ratio)

type Rational = Ratio Nat   -- used for type synonym expansion
type SRational (a :: Ratio Nat) = Sing a
...
$(singletonsOnly [d|
  ...
  divZeroError :: a
  divZeroError = error "divide by zero"

  {-# NOINLINE ratioZeroDenominatorError #-}
  ratioZeroDenominatorError :: a
  ratioZeroDenominatorError = error "zero denominator in Rational"
  ...
  -- no synonyms in singletons

  ratioPrec, ratioPrec1 :: Nat
  ...
  instance  Integral Nat  where
      ...
      a `quotRem` b
       ...
       | a < b     = (0, b)
       | otherwise = let (q,r) = (a-b) `quotRem` b in
                     (q+1,r)
